...
const blockDice = ["attacker-down", "both-down", "push", "defender-stumbles", "defender-down"];
class Viewer {
  constructor(socket) {
    ...
    socket.on('boardstate', function (msg) {
      ...
      if (this.boardState.activeTeam === 0) turn.classList.add("active-turn");

      turn = document.getElementById("awayTurn");
      turn.innerHTML = this.boardState.awayTeam.turn;
      if (this.boardState.activeTeam === 1) turn.classList.add("active-turn");
  ...
  init = () => {
    const getTeamDiv = (logo) => {
      ...
      e.style.setProperty('background-image', `url('https://cdn2.rebbl.net/images/logo/256x256/logo_${logo}.png')`);
  ...
  checkRerolls() {
    ...
    for (let x = this.boardState.homeTeam.rerolls; this.boardState.homeTeam.rerollsAvailable < x; x--) {
    ...
    for (let x = this.boardState.awayTeam.rerolls; this.boardState.awayTeam.rerollsAvailable < x; x--) {
      target.childNodes[target.childElementCount - x - 12].childNodes[0].classList.add("used");
    }
  }
  checkInducements() {
    ...
    if (this.boardState.homeTeam.apothecaryNumber === 0 && this.boardState.homeTeam.teamHasApothecary) {
      let d = document.createElement("div");
      d.classList.add("inducement");
      d.classList.add("apo");
    ...
    if (this.boardState.awayTeam.apothecaryNumber === 0 && this.boardState.awayTeam.teamHasApothecary) {
      let d = document.createElement("div");
      d.classList.add("inducement");
      d.classList.add("apo");
      d.classList.add("used");
  ...
  checkWeather() {
    ...
    ["heat", "sunny", "perfect", "rain", "blizzard"].map(x => this.cleanClass(x));
  ...
  getAvailableGridItem(id, state, side) {
    ...
    if (sprite && sprite.parentNode.id.startsWith(prefix)) return sprite.parentNode;
  ...
  createPlayer(p, css) {
    ...
    getPlayerType(p.type).split(" ").map(x => sprite.classList.add(x));
  ...
  selectPlayer(e) {
    ...
    const team = playerId > 30 ? this.boardState.awayTeam : this.boardState.homeTeam;
  ...
  placePlayer(p, css) {
    let sprite = document.getElementById(`player_${p.id}`);

    if (!sprite) {
      sprite = this.createPlayer(p, css);
    }
    ...
    switch (p.situation) {
      case 0:
        target = document.getElementById(`pos_${p.position.x}_${p.position.y}`);
        break;
      default:
        target = this.getAvailableGridItem(p.id, p.situation, css);
        break;
    }
    ...
    if (p.situation > 3) {
      ...
      image.style.setProperty("bottom", "-5px");
      image.style.setProperty("right", "-5px");
      if (p.situation === 4)
        image.src = `https://cdn2.rebbl.net/images/skills/${Casualties[Math.max(...p.sustainedCasualties)].icon}.png`
      else
        image.src = `https://cdn2.rebbl.net/images/skills/Expelled.png`
  ...
  updateClock() {
    ...
    const seconds = (time) => time > 3599
      ? `${~~(time / 3600)}:${~~(time % 3600 / 60) < 10 ? '0' : ''}${~~(time % 3600 / 60)}:${time % 60 < 10 ? "0" : ""}${time % 60}`
  ...
  handleActivate(state) {
    ["active", "dice", "cell", "d1v", "d1h", "d2v", "d2v", "d3v", "d3v", "attacker-down", "both-down", "defender-down", "defender-stumbles", "push", "foul"].map(cls => {
      let elements = document.getElementsByClassName(cls);
      while (elements.length > 0) {
    ...
    document.getElementById(`player_${state.playerId}`).parentElement.prepend(div);
    document.getElementById(`player_${state.playerId}`).classList.remove("prone");

    if (this.lastPlayerId > 0) {
      document.getElementById(`player_${this.lastPlayerId}`).classList.add("done");
    }
  ...
  handleBlock(state) {
    const spriteFrom = document.getElementById(`pos_${state.from.x}_${state.from.y}`).lastChild;
    let spriteTo = document.getElementById(`pos_${state.to.x}_${state.to.y}`).lastChild;
    ...
    if (state.rollType === 5) {
      ...
      if (state.completed) {
        ...
        div.classList.add(blockDice[state.diceResult[0]]);
        target.prepend(div);
      } else {
        let css = state.diceResult.length === 1 ? 'd1h' : state.diceResult.length === 2 ? 'd2h' : 'd3h';
    ...
    if (state.rollType === 13) {
      ...
      if (state.completed) {
        ...
        const target = document.getElementById(`pos_${state.cells[0].x}_${state.cells[0].y}`);
        if (this.lastChainPush) spriteTo = document.getElementById(this.lastChainPush);
        ...
        state.cells.map(cell => {
          ...
          if (cell.x < 0 || cell.x > 25 || cell.y < 0 || cell.y > 14) {
            //surf
            const target = this.getAvailableGridItem(state.playerId, 1, state.playerId < 21 ? "home" : "away");
            target.appendChild(sprite);
    ...
    if (state.rollType === 14) {
      //follow up
      if (state.completed) {
        document.getElementById(`pos_${state.from.x}_${state.from.y}`).childNodes[0].remove();
        const target = document.getElementById(`pos_${state.cells[0].x}_${state.cells[0].y}`);
  ...
  handleEndTurn(state) {
    ...
    const turn = document.getElementById(this.activeTeam === 0 ? "homeTurn" : "awayTurn");

    turn.innerHTML = this.activeTeam === 0 ? this.boardState.homeTeam.turn : this.boardState.awayTeam.turn;
  ...
  handlePickup(state) {
    ...
    if (state.completed && state.resultType === 0) //successful pickup
      sprite.classList.add("held");
  }
  ...
  handleTakeDamage(state) {
    ...
    switch (state.rollType) {
      case 3: //armor
        if (state.resultType === 1) { // armor failed
          //knocked down
          sprite.classList.add("prone");
        }
        break;
      case 4: //injury
        if (state.subResultType === 2) {
          ...
          //knocked out
          const target = this.getAvailableGridItem(state.playerId, 2, state.playerId < 21 ? "home" : "away");
          target.appendChild(sprite);
        }
        break;
      case 8: //casualty
        const target = this.getAvailableGridItem(state.playerId, 4, state.playerId < 21 ? "home" : "away");

        const casSprite = document.createElement("div");
        casSprite.classList.add("blood");
        casSprite.classList.add(`splatter${Math.floor(4 * Math.random() + 1)}`);
