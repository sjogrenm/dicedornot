<details on:click|preventDefault|stopPropagation={() => open = (!open)} {open}>
  ...
  <ul class="list-group" on:click|stopPropagation>
    <li class="list-group-item list-group-item-action">
      <details on:click|preventDefault|stopPropagation={() => actualOpen = (!actualOpen)} open={actualOpen}>
        <summary>Actual Outcomes</summary>
        {#if actualOpen}
        <ul class="list-group" on:click|stopPropagation>
          {#each actualDistributions as dist}
              <li class="list-group-item list-group-item-action"><Distribution dist={dist}/></li>
          {/each}
        </ul>
        {/if}
      </details>
